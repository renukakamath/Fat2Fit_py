...
public class Complaint extends Activity implements JsonResponse{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String q="/vcomplaint/?log_id="+log_id; 
		jr.execute(q);
		b1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				JsonReq jr= new JsonReq();
				jr.json_response=(JsonResponse) Complaint.this;
				String q="/scomplaint/?message="+message+"&log_id="+log_id;
				...
				if(method.equalsIgnoreCase("view")){
					...
					if(status.equalsIgnoreCase("success"))
					{
						...
						for(int i=0;i<ja.length();i++)
						{
							...
							messgages[i]=ja.getJSONObject(i).getString("description");
							reply[i]=ja.getJSONObject(i).getString("reply");
							
							val[i]="\nDate : "+date[i]+"\nComplaint : "+messgages[i]+"\nReply : "+reply[i];
						}
