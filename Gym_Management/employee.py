...
def employee_add_user_details():
	data={}
	q="SELECT * FROM `users` INNER JOIN `batches` USING(`batch_id`) INNER JOIN `gym_instructor` USING(`instructor_id`)"
	res=select(q)
	data['view_user']=res
...
def employee_add_attendance_details():
	...
	return render_template('employee_add_attendance_details.html',data=data)
